...
describe('EdStemApiClient', () => {
  ...
  it('replays a redacted live threads capture from profile 13 for private API coverage', async () => {
    ...
    if (result.ok) {
      ...
      expect(result.snapshot.courses?.find((item) => item.id === 'edstem:course:855')).toMatchObject({
        title: 'Allen School Career Board',
        code: 'redacted-course-code',
      });
      expect(result.snapshot.messages?.map((item) => item.id)).toEqual([
      ...
      expect(result.snapshot.messages?.[0]?.summary).toContain('Career-related events');
