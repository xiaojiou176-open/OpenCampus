...
test('cleanup:runtime removes repo-named temp residues across configured temp roots without touching non-campus paths', () => {
  ...
  try {
    ...
    writeFileSync(
      join(sandbox.repo, '.runtime-cache/live-fixtures/gradescope/submission.redacted.html'),
      '<html>redacted-fixture</html>',
      { encoding: 'utf8' },
    );
    writeFileSync(
      join(sandbox.repo, '.runtime-cache/browser-identity/index.html'),
    ...
    assert.equal(
      existsSync(join(sandbox.repo, '.runtime-cache/live-fixtures/gradescope/submission.redacted.html')),
      true,
    );
...
test('cleanup:runtime closeout mode removes fresh sensitive runtime residue without touching browser roots', () => {
  ...
  try {
    ...
    writeFileSync(
      join(sandbox.repo, '.runtime-cache/live-fixtures/gradescope/regrade-requests.redacted.html'),
      '<html>redacted-fixture</html>',
      { encoding: 'utf8' },
    );
    writeFileSync(
    ...
    assert.equal(
      existsSync(join(sandbox.repo, '.runtime-cache/live-fixtures/gradescope/regrade-requests.redacted.html')),
      false,
    );
