...
| DARS | degree-audit visibility | read-only only; current-user-only interpretation | summary-first partial shared landing through the existing `MyPlan` planning substrate. Current repo-side truth is a `Planning Pulse` / export / AI-caution summary lane, not a standalone DARS runtime site |
| Transcript | historical academic record summary | read-only only; summary-first and export-first | summary-first runtime lane now exists on the admin high-sensitivity substrate using the current transcript page carrier. Still not a standalone student-record domain or AI-readable detail lane |
| Financial aid | aid status summary | read-only only; summary-first and export-first | summary-first runtime lane now exists on the admin high-sensitivity substrate using the current financial-aid status page carrier. Still not a full aid-detail or document-ingestion lane |
| Accounts | account-state summary | read-only only; summary-first and export-first | summary-first runtime lane now exists on the admin high-sensitivity substrate using the current MyUW accounts page carrier. Still not a full billing/account-management domain |
...
| Course websites | `courses.cs.washington.edu` family (`Home / Syllabus / Schedule / Assignments`) | read-only only; summary-first and authority-aware | repo-side shared runtime lane now exists on the existing `Course / Assignment / Event / Resource` contracts for the current CS course-website family. Still expansion truth, not yet promoted to full current shipped scope or canonical merge truth |
...
| Transcript / finaid / tuition/accounts detail | `partial shared landing + deeper detail blocker` | current repo truth now has summary-first runtime carriers on the admin high-sensitivity substrate plus AI-deny rules, but there is still no truthful standalone detail/runtime carrier for the full records domain |
