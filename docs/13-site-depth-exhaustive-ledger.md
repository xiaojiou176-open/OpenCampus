...
| :-- | :-- | :-- | :-- |
| Canvas module-driven resource routing / groups / recordings | Could deepen course-material discovery beyond direct file metadata and syllabus summary | Current landed carrier only covers files plus syllabus summary resources; module trees, groups, and durable recording carriers still need a stronger truthful contract | `selective gap` |
...
| :-- | :-- | :-- |
| MyPlan | `partial shared landing` | shared planning substrate plus extension/web `Planning Pulse` adoption are real, and the current working tree now adds a repo-side capture path for `plan / audit` pages into that same substrate. Fresh canonical live/browser proof for that path still belongs to the manual Wave 3 lane. This is still not full shipped site/runtime parity or a public API claim |
...
| Transcript / finaid / accounts | `deepwater runtime landing` | summary-first runtime carriers now exist on the admin high-sensitivity substrate, but these lanes are still export-first and do not yet grant truthful full-detail/runtime parity or AI-readable records |
...
| Time Schedule | `partial shared landing` | valuable catalog/schedule context and now partially landed on the public course-offerings carrier, while still clearly separate from registration automation and full upstream parity |
