...
| `@campus-copilot/mcp` | `registry candidate` | `pnpm install` then `pnpm --filter @campus-copilot/mcp test` | `pnpm --filter @campus-copilot/mcp test` | [`../packages/mcp/README.md`](../packages/mcp/README.md), [`../examples/toolbox-chooser.md`](../examples/toolbox-chooser.md) | [`../examples/integrations/claude-code-mcp.example.json`](../examples/integrations/claude-code-mcp.example.json) | `public-ready (repo-local)` | no repo-local packaging blocker remains, but current repo proof does not itself prove a live npm publication page |
...
## Skill And Container Surfaces

| Surface | Public install path | Fresh proof loop | Public doc/router | Current state | What blocks official listing |
| :-- | :-- | :-- | :-- | :-- | :-- |
| Public skill pack catalog | [`../skills/catalog.json`](../skills/catalog.json) plus the matching `skills/*/SKILL.md` entrypoints and [`../skills/clawhub-submission.packet.json`](../skills/clawhub-submission.packet.json) | `pnpm check:skill-catalog` | [`../skills/README.md`](../skills/README.md), [`../INTEGRATIONS.md`](../INTEGRATIONS.md), [`skill-publication-prep.md`](skill-publication-prep.md), [`16-distribution-preflight-packets.md`](16-distribution-preflight-packets.md) | `public-ready (repo-local)` | the repo now ships a generic upstream packet, but the official docs still did not expose a stable marketplace manifest schema for plain skill uploads |
| Containerized API sidecar | `docker build -t campus-copilot-api:local .` then `docker compose up -d campus-copilot-api` | `pnpm smoke:docker:api` | [`../DISTRIBUTION.md`](../DISTRIBUTION.md), [`../packages/mcp-server/README.md`](../packages/mcp-server/README.md), [`container-publication.packet.json`](container-publication.packet.json), [`container-publication-prep.md`](container-publication-prep.md), [`16-distribution-preflight-packets.md`](16-distribution-preflight-packets.md) | `container-ready (repo-local)` | no container registry publication or platform-specific container listing has been completed |
...
- `@campus-copilot/mcp`, `@campus-copilot/cli`, `@campus-copilot/mcp-server`, `@campus-copilot/mcp-readonly`, `@campus-copilot/provider-runtime`, `@campus-copilot/gradescope-api`, `@campus-copilot/edstem-api`, and `@campus-copilot/myuw-api` are currently best described as `registry candidate` or `public-ready (repo-local)` until an upstream registry page is freshly confirmed
...
- the public skills now also ship with a repo-owned machine-readable catalog, but that catalog is still not the same thing as an upstream skill-registry manifest
- the repo now also ships a truthful Docker path for the thin local BFF, but that is still not the same thing as a published container image or hosted runtime
- the repo still does **not** have evidence for `officially listed` or `marketplace listed`
- package publication, repo-owned container path publication, marketplace/discovery work, and all official-listing language remain owner-controlled until an upstream page is freshly confirmed
...
| :-- | :-- | :-- | :-- | :-- |
| MCP Registry | Yes | [registry.modelcontextprotocol.io](https://registry.modelcontextprotocol.io/) and [registry quickstart](https://github.com/modelcontextprotocol/registry/blob/main/docs/modelcontextprotocol-io/quickstart.mdx) | `@campus-copilot/mcp-server` is the strongest bundled server artifact candidate because the official registry expects a published package plus `mcpName` and `server.json` metadata, and the repo now ships [`../packages/mcp-server/registry-submission.packet.json`](../packages/mcp-server/registry-submission.packet.json), [`16-distribution-preflight-packets.md`](16-distribution-preflight-packets.md), and `pnpm check:mcp-registry-preflight`. | An official MCP Registry submission still has to be executed; repo-local proof does not itself prove a live registry or npm page. |
...
| OpenClaw ClawHub / bundle face | Yes | [docs.openclaw.ai/tools/clawhub](https://docs.openclaw.ai/tools/clawhub) and [Plugin Bundles](https://docs.openclaw.ai/plugins/bundles) | The repo already ships a compatible Claude-style bundle layout via its existing `skills/` root, `skills/catalog.json`, read-only OpenClaw route, and [`../skills/clawhub-submission.packet.json`](../skills/clawhub-submission.packet.json). | No official OpenClaw / ClawHub publication has been completed yet, so the truthful state is still repo bundle rather than officially listed. |
| Glama MCP directory face | Yes | [glama.ai/mcp/servers](https://glama.ai/mcp/servers) | the repo now has a stronger repo-owned container path and stdio MCP path, and [`16-distribution-preflight-packets.md`](16-distribution-preflight-packets.md) records the generic packet set that is safe to reuse later | without a vendor-published submission/schema contract in this wave, the truthful repo action is to avoid inventing Glama-specific manifests and reuse the generic packet set instead |
