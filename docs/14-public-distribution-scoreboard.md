...
| :-- | :-- | :-- | :-- | :-- | :-- |
| Public skill pack catalog | [`../skills/catalog.json`](../skills/catalog.json) plus the matching `skills/*/SKILL.md` entrypoints | `pnpm check:skill-catalog` | [`../skills/README.md`](../skills/README.md), [`../INTEGRATIONS.md`](../INTEGRATIONS.md) | `public-ready (repo-local)` | this repo-owned catalog is intentionally not an upstream ClawHub or other marketplace manifest; upstream publication still needs the target platform's exact bundle flow |
...
| OpenClaw ClawHub / bundle face | Yes | [docs.openclaw.ai/tools/clawhub](https://docs.openclaw.ai/tools/clawhub) and [Plugin Bundles](https://docs.openclaw.ai/plugins/bundles) | The repo already ships a compatible Claude-style bundle layout via its existing `skills/` root, `skills/catalog.json`, and read-only OpenClaw route. | No official OpenClaw / ClawHub publication has been completed yet, so the truthful state is still repo bundle rather than officially listed. |
| Glama MCP directory face | Yes | [glama.ai/mcp/servers](https://glama.ai/mcp/servers) | the repo now has a stronger repo-owned container path and stdio MCP path, but this audit did not recover a stable vendor publish schema or verified submission contract for Campus Copilot | without a vendor-published submission/schema contract in this wave, the truthful repo action is to avoid inventing Glama-specific manifests |
