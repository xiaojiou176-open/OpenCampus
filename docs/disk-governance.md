...
## Repo-Owned Design Workspace

`.stitch/` is different from the local-only tooling directories above.

It is a repo-owned design workspace for committed design truth such as:

- `DESIGN.md`
- Stitch metadata
- generated design artifacts that are intentionally checked into the repo

Because of that, `.stitch/` must be treated as an allowed repo-root entry rather than local-only noise.
