...
const TEXT: Record<ResolvedUiLanguage, UiText> = {
  en: {
    ...
    options: {
      ...
      advancedRuntimeSettings: 'Advanced runtime settings',
      advancedRuntimeDescription:
        'Manual BFF overrides, model strings, and site-specific path overrides stay here so the first screen can stay summary-first.',
      manualFallbackOnly: 'Manual fallback',
  ...
  'zh-CN': {
    ...
    options: {
      ...
      advancedRuntimeSettings: '高级运行时设置',
      advancedRuntimeDescription:
        '手动 BFF 覆盖、模型字符串和站点级路径覆盖都下沉到这里，让第一屏继续保持 summary-first。',
      manualFallbackOnly: '手动兜底',
